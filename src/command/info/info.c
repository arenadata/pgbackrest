    ...
    VariantList *backupSection, InfoBackupData *backupData, const bool backupLabelMatch, InfoRepoData *repoData)
{
    FUNCTION_TEST_BEGIN();
        ...
        FUNCTION_TEST_PARAM(BOOL, backupLabelMatch);                // Does requested backup label match backupData.backupLabel?
    ...
    const unsigned int repoIdxMin, const unsigned int repoIdxMax)
{
    ...
    while (backupTotalProcessed < backupTotal)
    {
        ...
        if ((!cfgOptionTest(cfgOptType) || cfgOptionStrId(cfgOptType) == backupData.backupType) &&
            (backupLabel == NULL || strEq(backupData.backupLabel, backupLabel)))
            backupListAdd(backupSection, &backupData, backupLabel != NULL, repoData);
