...
testRun(void)
{
    ...
    if (testBegin("checkStanzaInfo(), checkStanzaInfoPg()"))
    {
        ...
        InfoBackup *backupInfo = infoBackupNew(PG_VERSION_96, 6569239123849665679, pgCatalogTestVersion(dbmsPostgreSQL, PG_VERSION_96), NULL);
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        backupInfo = infoBackupNew(PG_VERSION_96, 6569239123849665999, pgCatalogTestVersion(dbmsPostgreSQL, PG_VERSION_96), NULL);
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        backupInfo = infoBackupNew(PG_VERSION_95, 6569239123849665999, pgCatalogTestVersion(dbmsPostgreSQL, PG_VERSION_95), NULL);
        backupPg = infoPgData(infoBackupPg(backupInfo), infoPgDataCurrentId(infoBackupPg(backupInfo)));
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        backupInfo = infoBackupNew(PG_VERSION_95, 6569239123849665679, pgCatalogTestVersion(dbmsPostgreSQL, PG_VERSION_95), NULL);
        backupPg = infoPgData(infoBackupPg(backupInfo), infoPgDataCurrentId(infoBackupPg(backupInfo)));
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        infoBackupPgSet(backupInfo, PG_VERSION_96, 6569239123849665679, pgCatalogTestVersion(dbmsPostgreSQL, PG_VERSION_96));
        backupPg = infoPgData(infoBackupPg(backupInfo), infoPgDataCurrentId(infoBackupPg(backupInfo)));
